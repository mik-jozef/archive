...
mozna(int start){             /*  Funkcia naèíta èíslo od 1 do 30, vráti najblišie vyššie èíslo, kde premenná u(nacitane_cislo) = 0. Ak u30 = 1,  */
...
printf("vysl_*vysl_: %d, vloz: %d, pocet_vloz_znak:%d\n", vysl_prve * vysl_druhe, vloz, pocet_vloz_znak);

while(pocet_vloz_znak < 640){                              /*  Vkladá èísla dokım ich nebude aspoò 640 (10 virtuálnych (bez ukonèenia) riadkov krát 64 znakov) */
  ...
  printf("\nI'm here! pocet_vloz_znak:%d", pocet_vloz_znak);
}
